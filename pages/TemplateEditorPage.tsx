...
const TemplateEditorPage = () => {
  ...
  if (loading) return <WorkoutLoadingOverlay message={t.templateEditor.loading} />;
  ...
  return (
    <div className="relative p-4 max-w-lg mx-auto template-editor">
      ...
      <div className={`mb-6 glass card-dark p-4 flex items-center gap-4 header-container ${isScrolling ? 'scrolling' : ''}`}>
        ...
        <div className="flex-1 min-w-0 text-center">
          <h1 className="text-xl font-bold">
            {id ? t.templateEditor.editTemplate : t.templateEditor.newTemplate}
          </h1>
        </div>
      </div>
      <form
      ...
      >
        <div>
          <label htmlFor="template-name" className="block text-xl font-semibold template-label">{t.templateEditor.dayName}</label>
        ...
        <div className="space-y-4">
          <h2 className="text-xl font-semibold">{t.templateEditor.exercises}</h2>
          {exercises.map((ex) => (
            <div 
              key={ex._tempId} 
            ...
            >
              <div className={`${ex._state === 'deleting' ? 'hidden' : 'p-4 space-y-3'}`}>
                <div className="exercise-card-header mb-3">
                  <div className="name-wrap relative">
                    <textarea
                        ...
                        placeholder={t.templateEditor.exercisePlaceholder}
                        value={ex.name || ''}
                        onChange={(e) => {
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div>
                    <label className="text-xs template-sublabel">{t.templateEditor.setsLabel}</label>
                  ...
                  <div>
                    <label className="text-xs template-sublabel">{t.templateEditor.repsLabel}</label>
                    <input 
                      type="text" 
                      placeholder="10-12"
                      value={ex.reps || ''} 
                  ...
                  <div>
                     <label className="text-xs template-sublabel">{t.templateEditor.restLabel}</label>
      ...
      <ConfirmDialog
        ...
        title={t.templateEditor.deleteExercise}
        description={t.templateEditor.deleteExerciseDesc}
