        ...
        {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(day => {
          ...
          return (
            <div
              ...
              className="relative flex items-center justify-center h-10 w-10 sm:h-11 sm:w-11 md:h-12 md:w-12 lg:h-14 lg:w-14 rounded-full cursor-pointer transition-colors duration-200 mx-auto"
            >
              <span className={`flex items-center justify-center h-8 w-8 sm:h-9 sm:w-9 md:h-10 md:w-10 lg:h-12 lg:w-12 rounded-full ${isToday ? 'bg-blue-600 text-white' : 'hover:bg-gray-100'}`}>
                {day}
              </span>
              {hasWorkout && <div className="absolute bottom-1 h-1 w-1 bg-blue-500 rounded-full" />}
            </div>
          );
        })}
      </div>
